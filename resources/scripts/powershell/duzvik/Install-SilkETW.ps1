...
function Install-SilkETW
{
    ...
    param (
        ...
        [Parameter(Mandatory=$false)]
        [string]$SilkServiceConfigUrl = "https://raw.githubusercontent.com/OTRF/Blacksmith/master/resources/configs/SilkETW/SilkServiceConfig.xml"
    )
    ...
    if (!$DotNet_Check)
    {
        ...
        & C:\ProgramData\$ServiceName\v8\Dependencies\dotNetFx45_Full_setup.exe /q /passive /norestart
        start-sleep -s 5
    }
    ...
    if (!$MVC_Check)
    {
        ...
        & C:\ProgramData\$ServiceName\v8\Dependencies\vc2015_redist.x86.exe /q /passive /norestart
        start-sleep -s 5
    }
    ...
    $SilkServiceConfigPath = "C:\ProgramData\$ServiceName\v8\SilkService\SilkServiceConfig.xml"
